...
import { CaptureFinger, GetMFS100Info, VerifyFinger } from "../../../features/scanner";
...
const MessEntry = () => {
  ...
  const enterWithQrCode = async (completeRollHash) => {
    if (!completeRollHash) {
      toast.error("Please provide roll hash", {...toastOptions});
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        switch (response.status) {
          case 400:
            toast.error(
              data.message || "Bad request. Please check the input data", {...toastOptions}
            );
            break;
          case 401:
            toast.error("Invalid token! Please login again.", {...toastOptions});
            break;
          case 403:
            toast.error(data.message || "Only mess staff can enter students", {...toastOptions});
            break;
          case 404:
            toast.error(data.message || "User or mess details not found", {...toastOptions});
            break;
          default:
            toast.error("An unexpected error occurred. Please try again", {...toastOptions});
        }
      } else {
        toast.success(data.message || "Successfully entered the mess", {...toastOptions});
      }
    } catch (error) {
  ...
  const handleMessEntry = async (rollnumber, globalVariable, setMessage) => {
    ...
    try {
      ...
      const captureFingerprint = async () => {
        ...
        if (!isConnected) {
          toast.error("Cannot capture fingerprint; device not connected", {...toastOptions});
          return false;
        }
        ...
        if (!response.httpStatus) {
          ...
          toast.error("Fingerprint capture failed", {...toastOptions});
          return false;
        }
      ...
      const verifyFingerprintMatch = async (ansiTemplate, rollnumber) => {
        if (!ansiTemplate || !rollnumber) {
          toast.error(
            "Please capture fingerprint and enter a valid roll number!", {...toastOptions}
          );
          return false;
        }
        ...
        try {
          ...
          if (!userid) {
            toast.error("Roll number not found in the system",  {...toastOptions});
            return false;
          }
          ...
          if (!storedGalleryTemplate) {
            toast.error("No stored fingerprint found for this roll number", {...toastOptions});
            return false;
          }
          ...
          if (!matchResponse.httpStatus) {
            toast.error("Verification failed: " + matchResponse.err, {...toastOptions});
            return false;
          }

          if (!matchResponse.data.Status) {
            toast.error("Fingerprint does not match.", {...toastOptions});
            return false;
          }
          ...
          toast.error("Verification process failed: " + error.message, {...toastOptions});
          return false;
        }
      };
      ...
      const processMessEntry = async (rollnumber, globalVariable) => {
        try {
          ...
          toast.success("You can take this meal! Thanks for coming", {...toastOptions});
          setRollnumber("");
          return true;
        } catch (error) {
          // console.error("Error:", error);
  ...
  const renderFingerprintScanner = () => (
    <div className="fingerprint-scanner w-full">
      ...
      <button
        ...
        ${isLoading || !rollnumber ? "opacity-50 bg-[#136249] cursor-not-allowed" : "bg-[#00df9a]"}`}
      >
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="max-w-full mt-10 flex flex-col items-center bg-white p-6 m-2 rounded-xl shadow-[0_0px_8px_rgba(0,233,154)]">
        {/* <ToastContainer /> */}
          ...
          <button
            ...
            className={`px-4 py-2 ${
              selectedOption === "qrCode"
                ...
                : "border-2 border-[#00df9a] text-gray-700"
            } rounded-md`}
          >
            Use QR Code
          </button>
          ...
          <div className="qr-code-scanner w-full">
            ...
            <button
              ...
              className={`bg-blue-500 text-white rounded-md py-2 px-4 transition duration-300 ease-in-out w-full hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-400 ${
                isLoading ? "opacity-50 bg-[#136249] cursor-not-allowed" : " bg-[#00df9a] "
              }`}
              disabled={isLoading}
            >
